...
The search script lives inside this skill's own directory. Invoke it by its path **relative to the
project root** (where the SDD Kit sits). If you're in a subdirectory, `cd` to the project root first,
or prefix with the absolute project path:

```bash
python ".claude/skills/uiux-ui-ux-pro-max/scripts/search.py" "<query>" --domain <domain>
```

If `python` is not found, try `python3`, then `py -3`. Requires Python 3.x, no external dependencies.
**Nota SDD Kit:** o `/sdd-init` adiciona `python`/`python3` ao allowlist do `.claude/settings.json`
quando esta skill está presente — se o comando for bloqueado, confirme que o allowlist foi ajustado.
...
```bash
python ".claude/skills/uiux-ui-ux-pro-max/scripts/search.py" "<product_type> <industry> <keywords>" --design-system [-p "Project Name"]
```
...
**Example:**
```bash
python ".claude/skills/uiux-ui-ux-pro-max/scripts/search.py" "beauty spa wellness service" --design-system -p "Serenity Spa"
```
...
```bash
python ".claude/skills/uiux-ui-ux-pro-max/scripts/search.py" "<query>" --design-system --persist -p "Project Name" --output-dir "<project-root>"
```
...
```bash
python ".claude/skills/uiux-ui-ux-pro-max/scripts/search.py" "<query>" --design-system --variance <1-10> --motion <1-10> --density <1-10>
```
...
**Example:**
```bash
python ".claude/skills/uiux-ui-ux-pro-max/scripts/search.py" "internal analytics dashboard" --design-system --variance 8 --motion 7 --density 8 -p "Ops Console"
```

### Step 3: Supplement with Detailed Searches (as needed)

```bash
python ".claude/skills/uiux-ui-ux-pro-max/scripts/search.py" "<keyword>" --domain <domain> [-n <max_results>]
```
...
```bash
python ".claude/skills/uiux-ui-ux-pro-max/scripts/search.py" "<keyword>" --stack <stack>
```
...
python ".claude/skills/uiux-ui-ux-pro-max/scripts/search.py" "AI search tool modern minimal" --design-system -p "AI Search"

# Step 3: supplement
python ".claude/skills/uiux-ui-ux-pro-max/scripts/search.py" "search loading animation" --domain ux

# Step 4: stack guidelines
python ".claude/skills/uiux-ui-ux-pro-max/scripts/search.py" "suspense streaming bundle" --stack nextjs
```
