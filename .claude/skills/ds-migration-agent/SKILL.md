...
Before running any skill, check for a `sdd.config.md` (seção 12) file in the project root. If present, the agent passes integration and calibration settings to each skill in the chain:
...
- **Token source specification is critical.** The migration plan will enumerate sources (code files, Figma variables, or both). Confirm that `sdd.config.md` (seção 12) specifies `token_sources` — if it does not, the plan will note this as a dependency before rollout.
...
4. **Check `sdd.config.md` (seção 12):**
